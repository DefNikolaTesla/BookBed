...
export function openReader(bookId) {
  ...
  // Update tap zones visibility for epub pagination
  const tapZones = $("tap-zones");
  if (isEpub && state.settings.flow === "paginated") {
    tapZones.classList.remove("hidden");
  } else {
    tapZones.classList.add("hidden");
  }
...
function bindSettingsSheet() {
  ...
  $("sheet").querySelectorAll("[data-flow]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      // Update tap zones visibility
      const tapZones = $("tap-zones");
      if (currentBook?.format === "epub" && btn.dataset.flow === "paginated") {
        tapZones.classList.remove("hidden");
      } else {
        tapZones.classList.add("hidden");
      }
...
/* ---------- tap zones (paged epub) ---------- */
function initTapZones() {
  const tapZones = $("tap-zones");
  const tapPrev = $("tap-prev");
  const tapMid = $("tap-mid");
  const tapNext = $("tap-next");

  const updateTapZonesVisibility = () => {
    const isEpub = currentBook?.format === "epub";
    const isPaginated = state.settings.flow === "paginated";
  ...
  const origApply = window.__bookbedApplySettings;
  window.__bookbedApplySettings = (patch) => {
    if (origApply) origApply(patch);
    if (patch?.flow !== undefined) updateTapZonesVisibility();
  };

  // Initial
  updateTapZonesVisibility();
}
