...
export function openReader(bookId) {
  ...
  console.log("[BookBed] Opening reader, flow:", state.settings.flow);
  const events = {
    ...
    onSwiped: (direction) => {
      console.log("[BookBed] onSwiped called:", direction);
      if (direction === "left") void engine?.next();
      else void engine?.prev();
    },
