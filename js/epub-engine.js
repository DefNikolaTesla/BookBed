...
export class EpubEngine {
  constructor(file, container, settings, events) {
    ...
    this.handleMessage = (e) => {
      ...
      console.log("[BookBed] Received message:", data);
  ...
  injectTapHandler(contents) {
    ...
    console.log("[BookBed] injectTapHandler called for:", contents.document?.location?.href);
    let downX = 0;
    let downY = 0;
    let downT = 0;
    let moved = false;
    ...
    const onUp = (e) => {
      ...
      if (isSwipe) {
        console.log("[BookBed] Swipe detected:", dx < 0 ? "left" : "right", "dx:", dx, "dy:", dy);
        try {
  ...
  async next() {
    console.log("[BookBed] next() called");
    try {
      await this.rendition?.next();
    } catch (err) {
      console.error("[BookBed] next() error:", err);
    }
  }

  async prev() {
    console.log("[BookBed] prev() called");
    try {
      await this.rendition?.prev();
    } catch (err) {
      console.error("[BookBed] prev() error:", err);
    }
  }
