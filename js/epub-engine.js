...
export class EpubEngine {
  constructor(file, container, settings, events) {
    ...
    this.handleMessage = (e) => {
      const data = e.data;
      if (data?.source !== "bookbed" || data?.type !== "tap") return;
      const contents = this.rendition?.getContents?.() ?? [];
      const list = Array.isArray(contents) ? contents : [contents];
      if (!list.some((c) => c.window === e.source)) return;
      this.events.onTapped(data.zone);
    };
    void this.open();
  }
  ...
  injectTapHandler(contents) {
    ...
    const onMove = (e) => {
      if (Math.abs(e.clientX - downX) > 14 || Math.abs(e.clientY - downY) > 14)
        moved = true;
    };
    const onUp = (e) => {
      if (moved) return;
      ...
      const zone =
        e.clientX < w * 0.18 ? "left" : e.clientX > w * 0.82 ? "right" : "center";
      try {
